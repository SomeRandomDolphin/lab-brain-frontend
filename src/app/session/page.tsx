...
// useSearchParams() (used for /session?join=<id>) requires a Suspense
// boundary around whatever calls it, or `next build` fails prerendering
// this page with "useSearchParams() should be wrapped in a suspense
// boundary". The actual page logic moved into SessionPageInner; this
// default export just supplies that boundary. The fallback is brief —
// Suspense only holds it up long enough to read the search params on the
// client, not for anything slow like a network call.
...
function SessionPageInner() {
  ...
  // /session?join=<id> — the link generated by InviteToast/ControlBar's
  // "Copy link" buttons. JoinModal uses this to skip straight to the
  // PreCheck step (device check) with the session ID pre-filled, but still
  // requires an explicit "Join now" click there — see JoinModal.tsx. Read
  // once; we don't need to re-read it on every render.
  ...
  function handleHost(devices: DeviceChoice) {
    setHasEnded(false);
    start(devices);
  }
  ...
  function handleJoin(sid: string, devices: DeviceChoice) {
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="flex-1 overflow-hidden">
            {starting ? (
              ...
              <ErrorState
                error={error}
                onRetry={() => handleHost({ micEnabled: true, camEnabled: true })}
              />
            ) : (
              <VideoGrid />
            )}
